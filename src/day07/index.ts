...
const commands = puzzle.split("$ ").map((item) => {
...
commands.forEach(({ command, output }) => {
  ...
  //   console.log("before", { command, dirStack, cwd, sizesByDir });
  ...
  if (command) {
    if (command === "cd /") {
      ...
      if (!sizesByDir.has(cwd)) {
        sizesByDir.set(cwd, 0);
      }
      //   console.log(`+++ adding ${dirSize} to ${cwd}`);
      sizesByDir.set(cwd, sizesByDir.get(cwd) + dirSize);

      const dirStackCopy = [...dirStack];
      while (dirStackCopy.length > 0) {
        dirStackCopy.pop();
        const curr = `/${dirStackCopy.join("/")}`;
        // console.log(`+++ adding ${dirSize} to ${curr}`);
        sizesByDir.set(curr, sizesByDir.get(curr) + dirSize);
      }
    }
  }

  //   console.log("after", { command, dirStack, cwd, sizesByDir });
});

console.log({
  sizesByDir,
  answer: [...sizesByDir.values()].reduce(
